...
export class EntitySheetHelper {
  ...
  /**
   * Soumet la feuille Application V2 (équivalent de l’ancien _onSubmit).
   * @param {ApplicationV2} app
   */
  static async submitSheet(app) {
    const form = app.form;
    if (!form) return;
    const fd = new FormDataExtended(form);
    const flat = foundry.utils.flattenObject(fd.object);
    const updateData = EntitySheetHelper.updateGroups(
      EntitySheetHelper.updateAttributes(flat, app.document),
      app.document
    );
  ...
  static async createAttribute(event, app) {
    ...
    await EntitySheetHelper.submitSheet(app);
  }
  ...
  static async createAttributeGroup(event, app) {
    const a = event.currentTarget;
    const form = app.form;
    const prefixEl = a.parentElement?.querySelector(".group-prefix");
    const newValue = (prefixEl?.value ?? "").trim();
  ...
  static async deleteAttributeGroup(event, app) {
    ...
    const groupKeyInput = groupHeader?.querySelector(".group-key");
    const groupName = groupKeyInput?.value ?? "";
    const ok = await foundry.applications.api.DialogV2.confirm({
      window: { title: game.i18n.localize("SIMPLE.DeleteGroup") },
      content: `<p>${game.i18n.localize("SIMPLE.DeleteGroupContent")} <strong>${foundry.utils.escapeHTML(groupName)}</strong></p>`,
      rejectClose: false
    });
    if ( ok && groupContainer?.parentElement ) {
  ...
  static async createDialog(data={}, options={}) {
    ...
    // Render the confirmation dialog window (DialogV2 — Foundry V13+)
    return foundry.applications.api.DialogV2.prompt({
      ...
      ok: {
        ...
        callback: (event, button, dialog) => {
          ...
          const typeValue = typeField && "value" in typeField ? typeField.value : createData.type;

          const actorTemplate = collection.get(typeValue);
          if ( actorTemplate ) {
            createData = foundry.utils.mergeObject(actorTemplate.toObject(), createData);
            createData.type = actorTemplate.type;
            if ( createData.flags?.worldbuilding ) {
